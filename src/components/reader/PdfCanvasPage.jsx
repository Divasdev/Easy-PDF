...
export default function PdfCanvasPage({ pdf, pageNumber, settings, onVisible }) {
  ...
  useEffect(() => {
    ...
    const render = async () => {
      ...
      const key = `${pageNumber}-${settings.theme}-${settings.brightness}-${settings.contrast}-${Math.round(viewport.width)}`;
  ...
  }, [pdf, pageNumber, inView, settings.theme, settings.brightness, settings.contrast, settings.zoom]);
