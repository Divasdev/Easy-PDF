...
export default function PdfViewport({ pdf, settings, onPageChange, startPage = 1 }) {
  ...
  return <section ref={viewport} className="pdf-viewport" aria-label="PDF document">
    <div className="pdf-pages">
      {Array.from({ length: pdf.numPages }, (_, index) => <PdfCanvasPage key={index + 1} pdf={pdf} pageNumber={index + 1} settings={settings} onVisible={visible} />)}
    </div>
  </section>;
}
